...
static void App_SysTick_Init(void);
static void buttonConfig_Init(void);

/**
 * @brief  One-call board init: HAL, clock, GPIOs, UART, SysTick, button, IRQs.
 *         Call once at the start of main() before the super loop.
 */
...
static void App_SysTick_Init(void)
{
    syscfg.load  = (SystemCoreClock / 1000u) - 1u;
    syscfg.val   = 0u;
